...
public class IncludedClassFilesByBoth {
    ...
    public static List<String> readClassNamesFromTxtFiles(String txtDirPath) throws IOException {
        ...
        //System.out.println(classNames.toString() + ", " + classNames.size());
        return classNames;
    }
    ...
    public static String readHtmlFileUpToTfoot(File htmlFile) throws IOException {
        ...
        // Use JSoup to parse and format the HTML content
        Document doc = Jsoup.parse(content.toString());
        return doc.html(); // Return the formatted HTML as a string
    }
    ...
    public static int[] extractNumbersInHtml(String htmlContent) {
        ...
        // Extract the relevant row containing "Total"
        String totalRow = doc.select("tr:contains(Total)").html();
        if (totalRow != null) {
            // Regex to extract numbers, including commas
            Pattern pattern = Pattern.compile("([\\d,]+) of ([\\d,]+)");
            Matcher matcher = pattern.matcher(totalRow);
    
            if (matcher.find()) {
                // Remove commas from the numbers before parsing them
    ...
    public static void checkHtmlFiles(String htmlDirPath, List<String> txtClassNames) throws IOException {
        ...
        for (File htmlFile : htmlFiles) {
            ...
            if (found) { // Changed to FOUND since we are looking for covered classes and included in DepTrim
                ...
                if (!numbersMatch) {
                    //nrClassFiles++;
                    //System.out.println("HTML FILES: " + nrClassFiles);
                ...
                // Add the mismatched classes here and write to a different file
                // if (!numbersMatch) {
                //     //nrClassFiles++;
                //     //System.out.println("HTML FILES: " + nrClassFiles);
                //     mismatchMap.put(fullClassName, numbers); // Store mismatched class name and its numbers
            ...
            while (entries.hasMoreElements()) {
                ...
                if (mismatchMap.containsKey(jarEntry.getName())) {
                    ...
                    try (InputStream classStream = jarFile.getInputStream(jarEntry)) {
                        ...
                        String message = String.format("%s Included by both tools: %s - JACT (NotCovered, Total): %d, %d",
    ...
    public static void main(String[] args) {
        ...
        String logFilePath = "./Included_By_Both_Tools/woodstox.txt";
